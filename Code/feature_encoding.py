...
def subset(original_list: list, subset_list: list):
    """
    Checks if the pos tag sequence of a feature is a continuous subset of the pos tags of the entire sentence
...
def check_feature(sentence: str):
    """
    Checks the presence of features in the headlines and encodes them in a list
    ...
    if subset(pos_tags, ['DT', 'JJS', 'NN']):
        array_for_features[0] = 1

    if subset(pos_tags, ['DT', 'JJS', 'NNS']):
        array_for_features[1] = 1

    # feature 2
    if subset(pos_tags, ['NNP', 'POS', 'JJS', 'JJ', 'NN']):
        array_for_features[2] = 1

    # feature 11
    pattern_exists = re.findall("You won't believe", sentence)
    if pattern_exists:
        array_for_features[3] = 1

    pattern_exists = re.findall("You'll never guess", sentence)
    if pattern_exists:
        array_for_features[4] = 1

    if subset(pos_tags, ['PRP', 'MD', 'RB', 'VB']):
        array_for_features[5] = 1

    if subset(pos_tags, ['PRP', 'MD', 'RB', 'VB', 'WP']):
        array_for_features[6] = 1

    if subset(pos_tags, ['PRP', 'MD', 'RB', 'VB', 'DT']):
        array_for_features[7] = 1

    if subset(pos_tags, ['PRP', 'MD', 'RB', 'VB', 'WRB']):
        array_for_features[8] = 1

    # feature 12
    if subset(pos_tags, ['NNP', 'POS', 'JJS', 'JJ', 'NN']):
        array_for_features[9] = 1

    # feature 20
    if subset(pos_tags, ['DT', 'RBS', 'JJ', 'NN']):
        array_for_features[10] = 1

    # feature 21
    if subset(pos_tags, ['PRP$', 'JJ', 'NN']):
        array_for_features[11] = 1

    # feature 22
    if subset(pos_tags, ['CD', 'JJ', 'NN']):
        array_for_features[12] = 1

    # feature 23
    if subset(pos_tags, ['DT', 'NN']):
        array_for_features[13] = 1
